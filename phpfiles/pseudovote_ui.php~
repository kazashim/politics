...
if($min <=0)
$min=0;
$result=mysql_query("select vote_up,vote_down,name,C.cand_id from VOTE_COUNT as V,CAND_SIGNUP as C where V.constituency_id=$user_const_id and V.cand_id=C.cand_id order by vote_up desc ") or die(mysql_error());
...
//	echo $x;
	$response['cand_id'.$x]=$row['C.cand_id'];
